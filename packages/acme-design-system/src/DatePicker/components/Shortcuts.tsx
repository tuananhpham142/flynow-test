...
import { Period, ShortcutsItem } from '../DatePicker.types';
...
const ItemTemplate = React.memo((props: ItemTemplateProps) => {
    ...
    const getClassName: () => string = useCallback(() => {
        const textColor = TEXT_COLOR['dark'][primaryColor as keyof (typeof TEXT_COLOR)['dark']];
        const textColorHover = TEXT_COLOR.hover[primaryColor as keyof typeof TEXT_COLOR.hover];
        return `whitespace-nowrap w-1/2 md:w-1/3 lg:w-auto transition-all duration-300 hover:bg-grey-100 dark:hover:bg-white/10 p-2 rounded cursor-pointer ${textColor} ${textColorHover}`;
    }, [primaryColor]);
...
const Shortcuts: React.FC = () => {
    ...
    return shortcutOptions?.length ? (
        <div className='md:border-b mb-3 lg:mb-0 lg:border-r lg:border-b-0 border-grey-300 dark:border-grey-700 pr-1'>
