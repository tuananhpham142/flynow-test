...
import { Period } from '../../DatePicker.types';
...
const Days: React.FC<Props> = ({ calendarData, onClickPreviousDays, onClickDay, onClickNextDays }) => {
    ...
    const currentDateClass = useCallback(
        (item: number) => {
            ...
            if (formatDate(dayjs()) === formatDate(dayjs(itemDate)))
                return TEXT_COLOR['default'][primaryColor as keyof (typeof TEXT_COLOR)['default']];
            return '';
        },
    ...
    const activeDateData = useCallback(
        (day: number) => {
            ...
            if (dayjs(fullDay).isSame(period.start) && dayjs(fullDay).isSame(period.end)) {
                className = ` ${BG_COLOR['default'][primaryColor]} text-white font-medium rounded-full`;
            } else if (dayjs(fullDay).isSame(period.start)) {
                className = ` ${BG_COLOR['default'][primaryColor]} text-white font-medium ${
        ...
        (day: number) => {
            ...
            if (period.start && period.end) {
                if (dayjs(fullDay).isBetween(period.start, period.end, 'day', '[)')) {
                    return ` ${BG_COLOR['lighter'][primaryColor]} ${currentDateClass(day)} dark:bg-white/10`;
                }
            }

            if (!dayHover) {
                return `${className} ${BG_COLOR['hoverLighter'][primaryColor]}`;
            }

            if (period.start && dayjs(fullDay).isBetween(period.start, dayHover, 'day', '[)')) {
                className = ` ${BG_COLOR['lighter'][primaryColor]} ${currentDateClass(day)} dark:bg-white/10`;
            }
            ...
            if (dayHover === fullDay) {
                const bgColor = BG_COLOR['default'][primaryColor];
                className = ` transition-all duration-default text-white font-medium ${bgColor} ${
    ...
    const buttonClass = useCallback(
        (day: number, type: 'current' | 'next' | 'previous') => {
            ...
            return cn(baseClass, isDateDisabled(day, type) && 'line-through', 'text-grey-400');
        },
