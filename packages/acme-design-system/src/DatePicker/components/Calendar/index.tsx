...
const Calendar: React.FC<Props> = ({
    ...
    const clickMonth = useCallback(
        (month: number) => {
            setTimeout(() => {
                typeof changeMonth === 'function' && changeMonth(month);
    ...
    const clickYear = useCallback(
        (year: number) => {
            setTimeout(() => {
                typeof changeYear === 'function' && changeYear(year);
    ...
    return (
        <div className='w-full md:w-[296px] md:min-w-[296px]'>
            ...
            <div className='px-0.5 sm:px-2 mt-0.5 min-h-[285px]'>
                {showMonths && <Months currentMonth={calendarData.date.month() + 1} clickMonth={clickMonth} />}
