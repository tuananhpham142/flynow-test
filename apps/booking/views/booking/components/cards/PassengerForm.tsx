...
import { DateRangeType, DateValueType } from '@acme/design-system/DatePicker/DatePicker.types';
...
const PassengerForm: React.FC<Props> = (props) => {
    ...
    const getMinDate = () => {
        const date = new Date();
        if (PassengerType === PassengerEnum.Infant) {
            date.setFullYear(date.getFullYear() - 2);
        }
        if (PassengerType === PassengerEnum.Child) {
            date.setFullYear(date.getFullYear() - 12);
        }
        if (PassengerType === PassengerEnum.Adult) {
    ...
    const getStartDateFrom = () => {
        ...
        // }
        if (PassengerType === PassengerEnum.Child) {
            date.setFullYear(year - 7);
        }
        if (PassengerType === PassengerEnum.Adult) {
            date.setFullYear(year - 30);
        }
                            ...
                            return (
                                <div>
                                    ...
                                    <Datepicker
                                        maxDate={getMaxDate()}
                                        minDate={getMinDate()}
                                        startFrom={getStartDateFrom()}
                                        onChange={onChange}
                                        containerClassName={`border !rounded-sm ${
                                            error ? 'border-danger' : 'border-grey-400'
                                        }`}
                                        asSingle={true}
